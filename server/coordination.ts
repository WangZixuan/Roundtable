...
function profileTokens(text: string): string[] {
  return text.toLowerCase().split(/[^a-z0-9\u4e00-\u9fff]+/).filter((token) => token.length >= 2);
}

function profileScore(bot: CoordinationBot, haystack: string): number {
  const title = bot.title.toLowerCase();
  const profile = `${bot.name} ${bot.title} ${bot.description}`.toLowerCase();
  return [...new Set(profileTokens(haystack))].reduce((score, token) => {
    if (title === token || title.includes(token) || token.includes(title)) return score + 5;
    if (profile.includes(token)) return score + 2;
    return score;
  }, 0);
}

function taskRole(task: PlanTaskArtifact): CoordinationRole {
  return task.role?.trim() || task.assignee?.trim() || "contributor";
}

function taskProfileHaystack(task: PlanTaskArtifact): string {
  return `${task.role ?? ""} ${task.assignee ?? ""} ${task.title} ${task.description}`;
}

function bestProfileBot(bots: CoordinationBot[], haystack: string): CoordinationBot | undefined {
  if (bots.length === 0) return undefined;
  return [...bots].sort((a, b) => profileScore(b, haystack) - profileScore(a, haystack) || bots.indexOf(a) - bots.indexOf(b))[0];
}
...
export function normalizeCoordinationPlan(plan: PlanArtifact, goal: string, requireHighRiskReview = true, channelBots?: CoordinationBot[]): PlanArtifact {
  const tasks = plan.tasks.map((task) => {
    ...
    const role = task.role?.trim() || task.assignee?.trim() || bot?.title || "contributor";
    return {
      ...task,
      role,
      assignee: role,
...
export class CoordinationManager {
  ...
  private async execute(run: CoordinationRun, recovering = false): Promise<void> {
    const controller = new AbortController();
    ...
    try {
      ...
      if (!recovering || run.tasks.length === 0) {
        ...
        run.tasks = plan.tasks.map((task) => this.toRunTask(task, bindings.get(task.id), { planRevision: acceptedRevision }));
      ...
      while (true) {
        if (evaluateResults) await this.runResultDrivenDecisions(run, orchestrator, bots, controller.signal);
  ...
  private async runResultDrivenDecisions(
    ...
    for (let round = 1; round <= decisionLimit; round += 1) {
      ...
      try {
        ...
        const bound = this.bindRequestedBots(followup, [], bots);
        const planRevision = proposedPlanRevision;
        const fixCycle = trigger === "review_rejected" ? run.fixCycles + 1 : undefined;
        const added = bound.plan.tasks.map((task) => this.toRunTask(task, bound.bindings.get(task.id), {
          planRevision,
  ...
  private bindRequestedBots(
    ...
    for (const bot of requested) {
      ...
      let index = available.findIndex((task) => {
        const haystack = taskProfileHaystack(task);
        return bestProfileBot(bots, haystack)?.id === bot.id || profileScore(bot, haystack) > 0;
      });
      if (index < 0) index = 0;
    ...
    for (const task of tasks) {
      ...
      const bot = bestProfileBot(bots, taskProfileHaystack(task));
    ...
    const team = orchestrator.createTeam(teamName, {
      ...
      agents: plan.tasks.map((node) => {
        ...
        const instructions = [
          ...
          task.role === "reviewer" ? REVIEWER_PROMPT : "",
        ].filter(Boolean).join("\n");
        return {
          name: `worker-${task.id}`,
