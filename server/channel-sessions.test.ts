...
describe("Channel member sessions", () => {
  ...
  it("keeps tool activity in the agent session without projecting it into the channel", () => {
    const store = new Store(selection);
    const bot = store.createBot({ name: "Researcher" });
    const channel = store.createGroup("Project", [bot.id]);
    const session = store.ensureChannelSession(channel.id, bot.id)!;
    store.appendMessage(session.threadId, { role: "bot", kind: "activity", tool: { name: "read_file" } });
    store.appendMessage(session.threadId, { role: "bot", kind: "text", text: "Research complete" });

    expect(store.messagesFor(session.threadId)).toEqual(expect.arrayContaining([
      expect.objectContaining({ kind: "activity", tool: { name: "read_file" } }),
    ]));
    expect(store.messagesFor(channel.threadId)).toEqual([
      expect.objectContaining({ kind: "text", text: "Research complete" }),
    ]);
  });
