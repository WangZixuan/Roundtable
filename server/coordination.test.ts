...
describe("Coordinator domain", () => {
  ...
  it("binds tasks from Channel profile text when the plan omits botId", async () => {
    const dir = mkdtempSync(join(tmpdir(), "channel-profile-bind-")); dirs.push(dir);
    const crew: CoordinationBot[] = [
      { id: "research", name: "Rin", title: "Researcher", description: "Collect published evidence.", model: "m" },
      { id: "critic", name: "Cam", title: "Critic", description: "Challenge assumptions and evidence quality.", model: "m" },
    ];
    const manager = new CoordinationManager({
      file: join(dir, "runs.json"), groupBots: () => crew, coordinatorPolicy: policy,
      runCoordinatorTurn: async () => ({ text: JSON.stringify([
        { title: "Find evidence", description: "Assess published evidence", role: "Researcher" },
        { title: "Assess findings", description: "Challenge assumptions and summarize the conclusion", role: "Critic", dependsOn: ["Find evidence"] },
      ]) }),
      createTask: (id) => ({ threadId: id }),
      runBotTurn: async () => ({ text: "Evidence supports a limited feasibility conclusion." }),
    });
    const run = await manager.start("room", "Evaluate VLM feasibility");
    await vi.waitFor(() => expect(run.status).toBe("completed"));
    expect(run.tasks.map((task) => [task.role, task.botId, task.botName])).toEqual([
      ["Researcher", "research", "Rin"],
      ["Critic", "critic", "Cam"],
    ]);
    expect(run.roles).toEqual({
      Researcher: { botId: "research", botName: "Rin" },
      Critic: { botId: "critic", botName: "Cam" },
    });
  });
