...
export function RoutinesPage() {
  ...
  const dialogOpen = Boolean(editor || (liveSelected && selectedBot) || pausedOpen);
  useLayoutEffect(() => {
    setTitleBarBackdrop(dialogOpen ? 0.7 : 0);
    return () => setTitleBarBackdrop(0);
  }, [dialogOpen]);
  useEffect(() => {
      ...
      {state.routines.length === 0 && state.routineRuns.length === 0 ? (
        <div className="flex min-h-0 flex-1 items-center justify-center p-8">
          <div className="max-w-[430px] text-center">
            <div className="relative mx-auto mb-5 flex h-24 w-56 items-end justify-center gap-2">
