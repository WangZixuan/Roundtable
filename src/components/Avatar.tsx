...
/** Keep the multicolour knot recognisable while tinting it toward each bot. */
const ribbonColorsFor = (color: MausColor): [string, string, string] => {
  const fill = MAUS_COLORS[color] ?? MAUS_COLORS.green;
  return [
    mix(fill, "#8B5CF6", 0.68),
    mix(fill, "#3B82F6", 0.66),
    mix(fill, "#22C7A9", 0.62),
  ];
};
  ...
  return (
    ...
    >
      <CursorAvatar
        ...
        ribbonColors={ribbonColorsFor(color)}
        title={label ?? null}
