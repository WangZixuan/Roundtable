...
export type BotAvatarProps = {
  /** Composite channel avatars clip custom images within their own tiles. */
  ...
  label?: string;
  className?: string;
  // Retain main's presentation API; robot avatars deliberately stay static.
  state?: string;
  motion?: string;
  motionKey?: number;
  animated?: boolean;
...
/** Render a custom image when present and a static robot for every fallback case. */
export function BotAvatar({ bot, size = 44, label, className, imageShape = "circle" }: BotAvatarProps) {
  ...
  if (profile.avatarCrop === "initials" || !profile.avatarUrl || imageFailed) {
    return <RobotAvatar color={bot.color} size={size} label={label ?? bot.name ?? "Agent avatar"} className={className} />;
  }
